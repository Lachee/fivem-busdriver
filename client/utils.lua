...
SpawnPed = function(model, coords, callback)
    ...
    Citizen.CreateThread(function() 
        ...
        while not HasModelLoaded(modelHash) and attempts >= 0 do
            attempts = attempts - 1
            Citizen.Wait(50)
        end
        ...
        -- Failed to load
        if attempts <= 0 then 
            print('failed to load model in time', model)
            callback(nil) 
            return
        end

        -- Create the position and snap to ground
        local position = { x=coords.x, y=coords.y, z=coords.z }
        local heading = math.random() * 360
        if type(coords) == "vector4" then heading = coords.w end

        -- Snap to ground
        -- if coords.z == 0 then
        --     local onGround, groundZ = GetGroundZFor_3dCoord(position.x, position.y, position.z, false)
        --     if onGround then position.z = groundZ end
        -- end

        -- Create the ped and give the callback
        local ped = CreatePed(4, modelHash, position.x+.0, position.y+.0, position.z+.0, heading+.0, true, false)
        Citizen.Wait(10)
        callback(ped)
    end)
