...
Ped = {}
Ped.maxSpawnAttempts = 50  -- How many MS we are willing to wait for it to load
...
--- Spawns a ped on a new thread.
-- @params model the name of the model
-- @params coords the coordinates
-- @params callback the callback to be invoked when the function ends
...
Ped.NavigateTo = function(ped, coords, speed, withinRange, timeout)
    ...
    if speed >= Ped.TELEPORT then
        ...
        TaskFollowNavMeshToCoord(ped, coords.x+.0, coords.y+.0, coords.z+.0, spped, timeout, withinRange, false)
    end
end
...
Ped.EnterVehicle = function(ped, vehicle, seat, speed, timeout, priority)
    ...
    if speed >= Ped.TELEPORT then
        ...
        TaskEnterVehicle(ped, vehicle, timeout, seat, speed, 1, 0)
    end
